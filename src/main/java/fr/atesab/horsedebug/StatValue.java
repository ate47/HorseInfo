...
public class StatValue {
    ...
    public String getFormattedText(double value, String unit, boolean stared) {
        return (value >= getExcellentValue() ? ChatFormatting.GOLD
                : (value <= getBadValue()) ? ChatFormatting.RED : ChatFormatting.GREEN)
                + HorseDebugMain.significantNumbers(showFunction.apply(value))
                + unit
                + (stared ? (ChatFormatting.YELLOW + " " + HorseDebugMain.UTF8_STAR) : "");
    }
